...
from bot.utils.db.submissions import get_submissions
...
async def submissions_answer(message, val):
    ...
    if len(sub) > 0:
        for i in range(len(sub)):
            if i == 0:
                b.append(
                    f"<b><u>{sub[i].get('product')}</u></b>{chr(10)}<i>{sub[i].get('manager')}</i>{chr(10)}{sub[i].get('client')} {sub[i].get('different')}{chr(10)}"
                )
            if i > 0:
                if sub[i - 1].get("product") != sub[i].get("product"):
                    b.append(
                        f"<b><u>{sub[i].get('product')}</u></b>{chr(10)}<i>{sub[i].get('manager')}</i>{chr(10)}{sub[i].get('client')} {sub[i].get('different')}{chr(10)}"
                    )
                if sub[i - 1].get("product") == sub[i].get("product"):
